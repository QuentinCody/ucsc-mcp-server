...
export function registerGetSchema(server: McpServer, env?: SchemaEnv): void {
    const handler = createGetSchemaHandler("UCSC_DATA_DO", "ucsc");

    server.registerTool(
        "ucsc_get_schema",
        {
            title: "Get Staged UCSC Data Schema",
            description:
                "Get schema information for staged UCSC data. Shows table structures and row counts. " +
