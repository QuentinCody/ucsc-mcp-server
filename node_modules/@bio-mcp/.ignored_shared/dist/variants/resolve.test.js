...
import { COHORT_BUILD, buildKeyFor, buildVariantRecord, formatForCohort, liftover, lookupPosition, lookupRsid, parseVariant, parseVariantString, resolveRsid, resolveVariant, VariantResolutionError, } from "./resolve.js";
...
describe("formatForCohort", () => {
    it("uses canonical when present", () => {
        expect(formatForCohort(buildVariantRecord("1", 100, "A", "G"), "finngen")).toBe("1:100-A-G");
    });
...
describe("resolveVariant grch38 input", () => {
    ...
    it("throws not_found when no rsID exists at position", async () => {
        ...
        await expect(resolveVariant("grch38", "1-200-A-G", { fetchImpl })).rejects.toBeInstanceOf(VariantResolutionError);
    });
});
describe("liftover", () => {
    it("returns same coords when fromBuild === toBuild", async () => {
        const result = await liftover({ chr: "1", pos: 100, ref: "A", alt: "G" }, "GRCh38", "GRCh38");
    ...
    it("rounds-trips GRCh37 -> GRCh38 via Ensembl", async () => {
        ...
        const result = await liftover({ chr: "1", pos: 100, ref: "A", alt: "G" }, "GRCh37", "GRCh38", { fetchImpl });
    ...
    it("throws when target build has no mapping", async () => {
        ...
        await expect(liftover({ chr: "1", pos: 100, ref: "A", alt: "G" }, "GRCh37", "GRCh38", { fetchImpl })).rejects.toBeInstanceOf(VariantResolutionError);
    });
});
