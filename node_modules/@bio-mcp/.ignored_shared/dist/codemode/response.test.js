...
import { createCodeModeResponse, createCodeModeError, withCodeMode, ErrorCodes, } from "./response";
describe("createCodeModeResponse", () => {
    ...
    it("supports provenance in _meta", () => {
        ...
        const result = createCodeModeResponse({ gene: "TP53" }, { meta: { provenance } });
        const meta = result.structuredContent?._meta;
...
describe("withCodeMode", () => {
    it("wraps a successful function", async () => {
        const fn = async (args) => ({ name: args.id });
