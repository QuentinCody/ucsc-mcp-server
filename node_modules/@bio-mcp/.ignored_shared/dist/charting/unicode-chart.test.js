...
describe("renderUnicodeChart", () => {
    ...
    it("handles non-numeric values gracefully", () => {
        const result = renderUnicodeChart(makeSpec({ data: [{ label: "X", count: "not a number" }] }));
        // Should not throw, treats as 0
        expect(result).toContain("X");
    });
    it("handles single data point", () => {
        const result = renderUnicodeChart(makeSpec({ data: [{ label: "Only", count: 42 }] }));
        expect(result).toContain("Only");
        expect(result).toContain("42");
    });
});
//# sourceMappingURL=unicode-chart.test.js.map